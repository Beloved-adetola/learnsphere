...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Profile fetch kept for potential other uses, but removing organizationCode usage
      } catch (error) {
        toast({
          variant: "destructive",
  ...
  // handleSaveOrganizationCode removed
        ...
        <TabsContent value="manage" className="mt-6">
          <Card className="max-w-xl">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Instructor account: {currentUser?.email}
              </p>
              {/* Organization code setting removed as passwords are now set per-quiz */}
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
